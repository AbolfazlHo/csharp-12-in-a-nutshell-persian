...
در بالاترین سطح، انواع (types) در **namespace**ها سازمان‌دهی شده‌اند. بسیاری از انواع پرکاربرد — از جمله کلاس **Console** — در **System namespace** قرار دارند. کتابخانه‌های .NET در فضای نام‌های تودرتو **nested namespaces** سازمان‌دهی شده‌اند. برای مثال، **System.Text namespace** شامل انواعی برای مدیریت متن است و **System.IO** شامل انواعی برای ورودی/خروجی می‌باشد. 📚
...
مثال از اعضای نمونه: 
متد Convert در کلاس UnitConverter و متد ToString در نوع int.
...
بویژه در برنامه‌های بزرگ، منطقی است که انواع (Types) را در قالب فضای نام (namespace)‌ها سازمان‌دهی کنیم.
...
میزان دقیق این سربار وابسته به پیاده‌سازی داخلی زمان‌اجرای .NET است، اما حداقل 8 بایت است که برای ذخیره کلیدی به نوع شیء، و همچنین اطلاعات موقتی مانند وضعیت قفل برای چندنخی (Multy threading) و یک فلگ (Flag) برای مشخص کردن اینکه آیا شیء توسط garbage collector از جابجایی قفل شده یا خیر، استفاده می‌شود.
...
#### تقسیم (Division) /
+ در نوع‌های صحیح، نتیجه تقسیم همیشه بخش اعشاری را حذف می‌کند (گرد کردن به صفر به روش قطع کردن).
...
سی‌شارپ از عملگرهای بیتی زیر پشتیبانی می‌کند:

![Conventions-UsedThis-Book](../../assets/image/02/Table-2-3.png) 

عملگر شیفت به راست >> و تفاوتش با >>> 
...
3. استفاده از کاراکترهای ویژه (Escape Sequences):

همان کاراکترهای ویژه که برای char معتبر بودند، در string هم کار می‌کنند:
...
* کاراکترهای ویژه را نادیده می‌گیرد
...
#### دَرج‌شوندگی (درون‌دَرجی) رشته‌ها (String Interpolation)
اگر یک رشته با علامت $ شروع شود، به آن رشته درج‌شونده (درون‌درج) می‌گویند. این رشته‌ها می‌توانند شامل عبارات C# درون {} باشند:
...
```
**قابلیت‌های جدید C# در رشته‌های درج‌شونده (درون‌درج) (درجی)**

+ از C# 10:

 رشته‌های درون‌گذاری‌شده می‌توانند const باشند، به شرطی که تمام مقادیر درون‌گذاری نیز ثابت باشند:
...
+ از C# 11:
 ...
 رشته‌های درون‌گذاری‌شده می‌توانند چندخطی باشند (چه معمولی، چه verbatim):
...
پشته به‌صورت منطقی رشد می‌کند و کوچک می‌شود؛ یعنی وقتی یک متد یا تابع وارد می‌شود، فضا اضافه می‌شود و وقتی از آن خارج می‌شویم، آن فضا آزاد می‌گردد.
...
1. پیش از خواندن یک متغیر محلی (Local variable) حتماً باید مقداری به آن اختصاص داده شده باشد.
...
می‌توانید مقدار پیش‌فرض هر نوع داده‌ای را با استفاده از کلیدواژه‌ی default به دست بیاورید:

```c#
Console.WriteLine(default(decimal));   // 0
```

در صورتی که نوع داده قابل استنباط باشد، می‌توانید نوع را هم نیاورید:
```c#
decimal d = default;
```
...
توجه کنید که کلیدواژه‌ی ref هم در تعریف متد و هم در هنگام فراخوانی باید ذکر شود؛ این کار باعث می‌شود خیلی واضح باشد که آرگومان با مرجعش ارسال شده است.
...
1. متغیر پیش از ورود به متد لازم نیست مقداردهی شود.
...
```c#
Bar(x);     // درست است ✔ (همان متد با in صدا زده می‌شود)
Bar(in x);  // درست است ✔
```
**📌 نکته مهم**
...
8 / (4 / 2)   // نتیجه: 4
```
#### عملگرهای راست‌همبند (Right-associative operators) ⚡

در زبان #C، بعضی از عملگرها به‌جای چپ‌همبندی، راست‌همبند هستند. یعنی وقتی چند بار پشت سر هم بیایند، از راست به چپ ارزیابی می‌شوند.
...
این عملگر همچنین با انواع Nullable هم کار می‌کند (توضیح کامل در فصل "Nullable Value Types").
...
«اگر مقدار سمت چپ null بود، مقدار سمت راست را به آن منتسب کن.»
...
📌 این عملگر مخصوصاً در پیاده‌سازی ویژگی‌های محاسبه‌ای تنبل (lazy evaluation) خیلی کاربردی است.

#### عملگر Null-Conditional (Elvis) (?.)
...
**🌀 ترکیب با عملگر Null-Coalescing**

خیلی وقت‌ها ترکیب این دو عملگر کاربردی است:
```c#
System.Text.StringBuilder sb = null;
...
استفاده از آکولادها باعث می‌شود قصد شما شفاف‌تر و خوانایی کد در ساختارهای تو در تو بیشتر شود؛ حتی اگر کامپایلر به آنها نیازی نداشته باشد.
...
+ goto case x → پرش به یک case دیگر
...
✅ Switch Expressionها یک نسخه ساده‌تر و قدرتمندتر از switch هستند که:
...
حلقه‌ی foreach برای پیمایش روی عناصر یک شیء قابل شمارش (شمردنی) (Enumerable) استفاده می‌شود.
بسیاری از مجموعه‌ها در .NET مثل آرایه‌ها، لیست‌ها و حتی رشته‌ها (string) قابل شمارش  (شمردنی) (Enumerable) هستند.
