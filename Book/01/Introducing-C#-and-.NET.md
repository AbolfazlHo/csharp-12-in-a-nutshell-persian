...
در مدل سنتی شی‌گرایی، تنها نوع موجود کلاس (Class) است. اما در #C انواع دیگری هم وجود دارد، از جمله اینترفیس (Interface).
...
نوع‌های دیگری نیز وجود دارند مانند:
...
یعنی بررسی نوع داده‌ها نه فقط در زمان اجرا، بلکه در زمان کامپایل (ساخت برنامه) هم انجام می‌شود.
 ...
 این یعنی به‌جای اینکه فقط با نوشتن تست‌های واحد (Unit Tests) در زمان اجرا به دنبال خطاها باشید، کامپایلر خودش بررسی می‌کند که همۀ نوع‌ها در برنامه درست به‌کار رفته‌اند یا نه.

این ویژگی چند مزیت مهم داره:

+ برنامه‌های بزرگ راحت‌تر قابل مدیریت هستند.

+ کدها قابل پیش‌بینی‌تر و مطمئن‌تر (Robust) می‌شوند.

+ ابزارهایی مثل IntelliSense در ویژوال استودیو می‌توانند کمک کنند کدها را سریع‌تر و بهتر بنویسید، چون دقیقاً می‌دانند هر متغیر از چه نوعی است و چه متدهایی می‌تواند روی آن اجرا شود.

+ همچنین این ابزارها می‌توانند بررسند که یک متغیر یا متد یا نوع، در کجای برنامه استفاده شده — که این موضوع برای بازسازی و تغییر ساختار کد (Refactoring) بسیار مهم است.

**تایپ پویا یا dynamic**

در عین حال که #C عمدتاً یک زبان با تایپ ایستاست، اجازه می‌دهد که بخش‌هایی از کد را به‌صورت پویا تایپ کنید.
برای این کار می‌تونید از کلیدواژه‌ی dynamic استفاده کنید؛
با این وجود، #C در ذات خود همچنان یک زبان با تایپ ایستا باقی می‌ماند.
...
مثلاً:
شما نمی‌توانید تابعی که انتظار دارد یک عدد صحیح (int) دریافت کند، با یک عدد اعشاری (float) صدا بزنید — مگر اینکه صراحتاً عدد اعشاری را به عدد صحیح تبدیل (Cast) کرده باشید.

این موضوع از بروز خطاهای رایج و پنهان می‌کاهد.

✅ این ویژگی‌ها باعث می‌شوند زبان #C هم قدرتمند باشد و هم امن و قابل اعتماد — مخصوصاً برای ساخت برنامه‌های بزرگ و پیچیده.
...
با این حال
 زبان #C اشاره‌گرها را کاملاً حذف نکرده؛ فقط استفاده از آن‌ها را برای اکثر کارها غیرضروری کرده است.
مگر در برخی موارد خاص که:

+ عملکرد (Performance) خیلی حیاتی باشد

+  نیاز به تعامل با کدهای Native یا کتابخانه‌های سطح پایین باشد
...
در استفاده روزمره، بدون اشاره‌گر هم می‌توانید همه کارها را به‌سادگی انجام دهید.
...
📊 این ساختار معمولاً در قالب نموداری مثل شکل 1-1 نمایش داده خواهد شد.
...
## زمان اجرای مشترک (CLR) (Common Language Runtime)
...
## کتابخانه کلاس پایه (Base Class Library)
...
+ برنامه‌های وب و خط فرمان (Console) برای سیستم‌عامل‌های
...
+ برنامه‌های دسکتاپ با رابط کاربری غنی (Rich-Client Applications) برای
...
📘  تمرکز اصلی این کتاب بر  CLR و BCL در .NET 8 است.
...
برای نوشتن برنامه‌های دسکتاپ با رابط کاربری غنی (Rich-Client) که روی ویندوز 10 و نسخه‌های جدیدتر اجرا شوند، می‌توانید از بین دو گزینه انتخاب کنید:
...
این باعث شده که نامگذاری جدید مایکروسافت (که .NET Core را به .NET تغییر داده) گاهی اوقات گیج‌کننده باشد.
...
از نسخه #C 12 به بعد، می‌تونید لیست پارامترهای سازنده را مستقیم بعد از تعریف کلاس یا استراکت بنویسید.
برای مثال:
...
اما در recordها، کامپایلر به‌صورت پیش‌فرض برای هر پارامتر یک property عمومی فقط‌خواندنی (init-only) هم می‌سازد؛ اما در کلاس‌ها و استراکچرها این اتفاق نمی‌افته.
...
Point p = (3, 4);
```
✅ این باعث می‌شه کد خواناتر و قابل نگهداری‌تر شود — مخصوصاً وقتی از نوع‌های پیچیده به دفعات استفاده می‌کنید.
...
```
✅ این ویژگی کمک می‌کند نیازی به نوشتن سازنده‌هایی با پارامترهای زیاد نداشته باشید،
که این موضوع در کلاس‌های فرزند (Subclasses) ساده‌سازی بزرگی به‌حساب می‌آید.

🔸 همچنین اگر بخواهید هم سازنده (Constructor) بنویسید و هم از required استفاده کنید،
می‌توانید از اتریبیوت [SetsRequiredMembers] روی سازنده استفاده کنید تا از محدودیت عبور کنید.
...
از نسخه C# 11، امکان جدیدی اضافه شده است که به شما اجازه می‌دهد درون یک Interface، متدهای استاتیک با نوع virtual یا abstract تعریف کنید.
...
می‌توانید آن namespace را روی تمام فایل‌های پروژه اعمال کنید:
...
### ✍️ تغییر بدون تخریب روی نوع‌های ناشناس (Nondestructive Mutation for Anonymous Types)
...
### 🧱 مقداردهی اولیه فیلدها و سازنده‌ی بدون پارامتر در Structها (Field Initializers and Parameterless Constructors in Structs)
...
🔸 اگر پارامتر داشته باشید، باید نوع آن را صراحتاً مشخص کنید:
...
اکنون می‌توانید lambda را به‌عنوان آرگومان به متدهایی پاس دهید که نوع پارامترشان object یا Delegate یا Expression است:
...
اگر بخواهید عبارت اصلی‌ای را بگیرید که برای یک پارامتر متد استفاده شده  می‌توانید از CallerArgumentExpression استفاده کنید:
...
🧵 رشته‌های دَرجی (Interpolated Strings) می‌توانند ثابت (const) باشند
به شرطی که مقادیری که درون‌شان استفاده شده هم const باشد:
...
اکنون می‌توانید شماره ستون و بازه (range) هم تعیین کنید — مخصوصاً برای ابزارهای آنالیز کد مفید است.

📛 در Recordها می‌توانید متد ToString() را ببندید (seal کنید)
یعنی نگذارید کلاس‌های مشتق‌شده بتوانند آن را override کنند:
...
در نسخه‌های قبلی C#، این کد باعث خطا می‌شد چون کامپایلر فکر می‌کرد ممکن است متغیر number مقدار نگرفته باشه:
...
🔸 سی‌شارپ (C#) 9.0 همراه با Visual Studio 2019 عرضه شد و زمانی استفاده می‌شود که پروژه‌ی شما برای .NET 5 هدف‌گذاری شده باشد.

**🧾 دستورات سطح بالا (Top-Level Statements)**

با استفاده از Top-Level Statements، می‌توانید برنامه‌ای بنویسید بدون اینکه مجبور باشید کلاس Program و متد Main را تعریف کنید:

```csharp
using System;
Console.WriteLine("Hello, world");
```
✅ این ویژگی باعث می‌شود کدهای نمونه و آموزشی بسیار ساده‌تر و مختصرتر نوشته شوند.

🔍 نکات تکمیلی:

+ می‌توانید در این بخش، متد هم تعریف کنید (مثل متد محلی یا local method).

+ به متغیر جادویی args برای گرفتن آرگومان‌های خط فرمان دسترسی دارید.

+ می‌توانید مقداری را به فراخواننده (caller) برگردانید.

+ بعد از دستورات سطح بالا، همچنان می‌توانید کلاس‌ها و فضای نام (namespace) تعریف کنید.
...
```
این یعنی ویژگی فقط در زمان مقداردهی اولیه قابل تنظیم است و بعد از آن فقط خواندنی است.

👨‍💻 استفاده از init به شما این امکان را می‌دهد که نوع‌هایی تغییرناپذیر (immutable) بسازید و با استفاده از initializer آن‌ها را مقداردهی کنید، بدون نیاز به تعریف چندین سازنده (constructor):

```csharp
var foo = new Foo { ID = 123 };
```
🔄 در کنار رکوردها (records)، این قابلیت باعث می‌شود بتوانید تغییرات غیرویرانگر (non-destructive mutation) انجام دهید؛ یعنی به جای تغییر شیء فعلی، یک نمونۀ جدید بسازید با مقادیر تغییر یافته.
...
```
🔸 تعریف کلاس رکورد ما می‌تواند به این شکل باشد:
...
```
🔹 در موارد ساده، رکوردها می‌توانند ما را از نوشتن کدهای تکراری (مثل تعریف ویژگی‌ها، سازنده و deconstructor) بی‌نیاز کند. برای مثال، تعریف بالا را می‌توانیم این‌طور خلاصه کنیم بدون از دست دادن قابلیت‌ها:

```csharp
record Point(double X, double Y);
```
📌 رکوردها مانند تاپل‌ها (tuples)، به طور پیش‌فرض از برابری ساختاری (structural equality) استفاده می‌کنند، نه فقط برابری مرجع (reference equality).

+ رکوردها می‌توانند از رکوردهای دیگر ارث‌بری کنند.

+ می‌توانند شامل همۀ اجزایی باشند که کلاس‌ها می‌توانند داشته باشند.

+ در زمان اجرا، توسط کامپایلر به کلاس تبدیل می‌شوند.

**🧩 بهبودهای تطبیق الگو (Pattern Matching)**

**🔸 الگوهای رابطه‌ای (Relational Patterns)**

از نسخه 9.0، می‌توانید عملگرهای <، >، <= و >= را درون الگوها استفاده کنید.
...
اکنون می‌توانید الگوها را با استفاده از کلمات کلیدی and، or و not ترکیب کنید:
...
+ می‌توانی با استفاده از پرانتزها اولویت را تغییر دهی.

🔍 از not هم می‌توانید با الگوی نوع (type pattern) استفاده کنید، برای مثال بررسی اینکه یک شیء از نوع خاصی نیست:
...
در C# 9.0 می‌توانید در جایی که کامپایلر می‌تواند نوع را حدس بزند، اسم نوع را در new حذف کنید:

```csharp
System.Text.StringBuilder sb1 = new();
System.Text.StringBuilder sb2 = new("Test");
```
📌 این ویژگی زمانی بسیار مفید است که تعریف متغیر و مقداردهی اولیه در دو بخش متفاوت از کد باشد:
...
```
یا مثلاً وقتی می‌خواهید مستقیماً به متد آرگومان بدهید:
...
سی‌شارپ C# 8 امکان افزودن پیاده‌سازی پیش‌فرض به اعضای واسط‌ها (interfaces) را فراهم کرده است. این بدان معناست که پیاده‌سازی آن عضو، برای کلاس‌هایی که از آن واسط استفاده می‌کنند اختیاری است:
...
تا پیش از C# 8، می‌توانستید با استفاده از yield return یک ایتراتور (پیماینده) (iterator) بنویسید یا با استفاده از await یک تابع ناهمزمان (asynchronous function). اما نمی‌توانستید هر دو را با هم ترکیب کنید — یعنی ایتراتوری (پیماینده‌ای) بنویسید که درون آن await استفاده شود و مقدارها را به‌صورت ناهمزمان بازگرداند.
...
سی‌شارپ C# 7.x نخستین بار همراه با Visual Studio 2017 منتشر شد. نسخه C# 7.3 همچنان در Visual Studio 2019 استفاده می‌شود، زمانی که هدف شما .NET Core 2، .NET Framework 4.6 تا 4.8 یا .NET Standard 2.0 باشد.
...
علاوه بر این‌ها، C# 2.0 ویژگی‌های دیگری را نیز اضافه کرد. مانند:
